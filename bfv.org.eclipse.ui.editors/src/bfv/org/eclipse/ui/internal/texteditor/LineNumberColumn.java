 ...
 *******************************************************************************/
package bfv.org.eclipse.ui.internal.texteditor;
...
import bfv.org.eclipse.ui.internal.texteditor.PropertyEventDispatcher;
import bfv.org.eclipse.ui.internal.texteditor.RulerMessages;
import bfv.org.eclipse.ui.texteditor.AbstractDecoratedTextEditor;
...
import bfv.org.eclipse.ui.texteditor.BfvLineNumberRulerColumn;

/**
 * The line number ruler contribution. Encapsulates a {@link NewLineNumberChangeRulerColumn} as a
...
public class LineNumberColumn extends AbstractContributedRulerColumn implements IVerticalRulerInfo, IVerticalRulerInfoExtension {
	...
	public static final String ID= "bfv.org.eclipse.ui.editors.columns.linenumbers"; //$NON-NLS-1$
	...
	private void updateForegroundColor(IPreferenceStore store, IVerticalRulerColumn column) {
		...
		if (column instanceof BfvLineNumberRulerColumn)
			((BfvLineNumberRulerColumn) column).setForeground(sharedColors.getColor(rgb));
	}
	...
	private void updateBackgroundColor(IPreferenceStore store, IVerticalRulerColumn column) {
		...
		ISharedTextColors sharedColors= getSharedColors();
		if (column instanceof BfvLineNumberRulerColumn)
			((BfvLineNumberRulerColumn) column).setBackground(sharedColors.getColor(rgb));
	}
	...
	private void updateCharacterMode(IPreferenceStore store, IVerticalRulerColumn column) {
		if (column instanceof NewLineNumberChangeRulerColumn) {
			NewLineNumberChangeRulerColumn lncrc= (NewLineNumberChangeRulerColumn) column;
	...
	private void updateLineNumbersVisibility(IVerticalRulerColumn column) {
		if (column instanceof NewLineNumberChangeRulerColumn)
			((NewLineNumberChangeRulerColumn) column).showLineNumbers(getLineNumberPreference());
	}
	 ...
	 */
	public void initializeLineNumberRulerColumn(BfvLineNumberRulerColumn rulerColumn) {
		IPreferenceStore store= getPreferenceStore();
		if (store != null) {
	...
	public boolean isShowingLineNumbers() {
		return fDelegate instanceof NewLineNumberChangeRulerColumn && ((NewLineNumberChangeRulerColumn) fDelegate).isShowingLineNumbers();
	}
	...
	public boolean isShowingChangeInformation() {
		return fDelegate instanceof NewLineNumberChangeRulerColumn && ((NewLineNumberChangeRulerColumn) fDelegate).isShowingChangeInformation();
	}
	...
	public boolean isShowingRevisionInformation() {
		if (fDelegate instanceof NewLineNumberChangeRulerColumn)
			return ((NewLineNumberChangeRulerColumn) fDelegate).isShowingRevisionInformation();
		return false;
	}

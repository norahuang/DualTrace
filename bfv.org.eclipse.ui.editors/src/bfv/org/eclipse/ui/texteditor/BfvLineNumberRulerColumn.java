 ...
 * @since 2.0
 */
public class BfvLineNumberRulerColumn implements IVerticalRulerColumn {
	 ...
	 */
	public BfvLineNumberRulerColumn() {
	}
	...
	void doPaint(GC gc, ILineRange visibleLines) {
		...
		for (int line= visibleLines.getStartLine(); line < lastLine; line++) {
			...
			if (widgetLine == -1) {
				...
				int nextLine = getEndOfCurrentCollapsedRegion(line);
				if(nextLine >= 0) {
					line = nextLine;
				}
	...
	/**
	 * Tells this class what the current collapsed ranges in the view are
	 */
	public void setCurrentCollapsedRegions(List<ILineRange> collapsedRanges) {
		this.fCollapsedRanges = collapsedRanges;
	}
	
	public int getEndOfCurrentCollapsedRegion(int currentLine) {
		
		if(fCollapsedRanges == null) {
			return -1;
		}
		
		for(ILineRange lineRange : fCollapsedRanges) {
			int endLine = lineRange.getStartLine() + lineRange.getNumberOfLines() - 1;
			
			if(currentLine >= lineRange.getStartLine() && currentLine <= endLine) {
				return endLine;
			}
		}

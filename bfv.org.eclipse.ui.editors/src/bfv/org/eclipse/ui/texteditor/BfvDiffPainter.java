 ...
 * @since 3.2
 */
public final class BfvDiffPainter {
	/**
	...
	private List<ILineRange> fCollapsedRanges;
	 ...
	 *        <code>null</code>
	 */
	public BfvDiffPainter(IVerticalRulerColumn column, ISharedTextColors sharedColors) {
		Assert.isLegal(column != null);
		fColumn= column;
		fSharedColors= sharedColors;
		fCollapsedRanges = new ArrayList<ILineRange>();
	}
	...
	public void paint(GC gc, ILineRange visibleModelLines) {
		...
		for (int line= visibleModelLines.getStartLine(); line < lastLine; line++) {
			...
			int endLine;
			if((endLine = getEndlineOfCollapsedRegion(line)) != -1) {
				line = endLine;
				continue;
			}
			
			// if line in a collapsed region and is not the first line in that region
			paintLine(line, gc, width, deletionColor);
		}
	}

	/**
	 * If the line is in a collapsed region, it returns the last line in that region.  Otherwise it returns -1
	 */
	private int getEndlineOfCollapsedRegion(int line) {
		
		for(ILineRange collapsedRegion : fCollapsedRanges) {
			int endLine = collapsedRegion.getStartLine() + collapsedRegion.getNumberOfLines() - 1;
			
			if(line > collapsedRegion.getStartLine() && line <= endLine) { 
				return endLine;
			}
		}
	...
	public void setCurrentCollapsedRegions(List<ILineRange> collapsedRanges) {
		this.fCollapsedRanges = collapsedRanges;		
	}

...
public class LoadLibraryExportsDialog extends TitleAreaDialog {
	...
	public void create() {
		...
		setMessage("Load the exported symbols from a DLL file and attempt to match them to functions in the trace.",
	...
	protected Control createDialogArea(Composite parent) {
		...
		AtlantisFileModelDataLayer fileModel = (AtlantisFileModelDataLayer) RegistryUtils
				.getFileModelDataLayerFromRegistry();
		for (ModuleRec module : fileModel.getAllModules()) {
			if (0 == module.versionInfoUnion && "" == allModuleInfo) {
				allModuleInfo = "***********************************************************************\r"
						+ "**No module versioning info available, this is an old trace format.**\r" + // Looks
																										// short,
																										// it's
																										// not.
						"***********************************************************************\r" + "\r";
			}
			allModuleInfo += module.winNameValue + "\r\t- version: " + module.versionInfoUnion + "\r\t- timestamp: "
					+ Long.toString(module.lastFileWriteTimestamp) + "\r";
		}
			...
			dialog.run(true, true, new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					...
					try {
						...
						// skip to the offset to the signature (skip to the
						// first element in the PE header)
						inputFile.skipBytes(0x3C);

						// read the signture offset (will be different than the
						// signature found at the onset of the DOS header)
						long signatureOffset = readUnsignedInt32(inputFile); // (aka
																				// e_lfanew,
																				// end
																				// of
																				// DOS
						...
						// skip the first 2 bytes of the COFF header (skip the
						// Machine entry)
						...
						// Needed for comparison to binary format reported
						// timeDateStamp
						...
						// skip the next 8 bytes of the COFF header to get to
						...
						// skip the end of the COFF header, skipping
						// Characteristics
						inputFile.skipBytes(2);
						...
						// Murray found PE32 Plus docs elsewhere, but I found
						// this: http://stackoverflow.com/a/15608028/682536
						// Require this flag to distinguish between 32 and 64
						// bit assemblies.
						int magicNumber = readUnsignedInt16(inputFile);
						boolean PE32Plus = magicNumber == 0x020B;

						// After getting the version items that *were not* part
						// of Murray's original parse,
						// we need to account for the actual position in the
						// file prior to taking the
						// next blind skipBytes() call. Those calls, dependent
						// on PE32Plus, assume that we *just* finished
						// reading magicNumber, which resides at 0x0018.
						long afterMagicFilePointer = inputFile.getFilePointer();

						// skip to the versions. We need four to correspond to
						// the moduleRec versionInfoUnion,
						// but I am not sure which quartet makes that up:
						// 2 bytes MajorOperatingSystemVersion,
						// MinorOperatingSystemVersion at 0x0040
						// 2 bytes MajorImageVersion, and MinorImageVersion at
						// 0x0044
						// 2 bytes MajorSubsystemVersion, MinorSubsystemVersion
						// 0x0048
						// 4 bytes Win32VersionValue (zeros filled) 0x0052
						// The Win32VersionValue cannot be the version of
						// interest, since we need four 16 bit numbers.
						// I will try to confirm, but otherwise implement as the
						// OS and Image versions.
						// We have to keep track of what we have read and
						// skipped so far, since Murray decided not have access
						// to the random-access capable HugeBitBuffer at the
						// time of this implementation.
						// NB This is all fragile code, tread lightly.

						// Found that until after the version number info, the
						// byte positions for things in the
						// Windows Specific Header are the same between 32 and
						// 64 bit versions, therefore between PE32 and PE32+.
						// https://code.google.com/archive/p/corkami/wikis/PE102.wiki
						// Added pdf from that to repo. Note that "18+60+ means
						// offset by 0x18 bytes, length 0x60 bytes.
						// Offsets for the versions from the pdf called
						// PE102[booklet|poster]V1.pdf are:
						...
						// 34+4 Win32VersionValue (cannot be the version number
						// of interest for the binary format, too big)
						// These do not correspond with the offsets in
						// Portable_Executable_32_bit_Structure_in_SVG.svg,
						// which
						// I found on wikipedia and added to the repo as well.
						// I will use the values from the more comprehensive pdf
						// instead, but I will commit the svg as well.
						...
						// on PE32Plus, assume that we *just* finished
						// reading magicNumber, which resides at 0x0018.
						// The easiest would be to seek to the point Mu was
						// arbitrarily jumping from, after Magic.
						inputFile.seek(afterMagicFilePointer);

						// skip the standard fields in the optional header
						inputFile.skipBytes(PE32Plus ? 22 : 26);

						// skip the all but the last window specific field in
						// the optional header
						...
						// if we have less than 1, then we have no exports
						// section
						...
						// we are now in the section headers, search for
						// ".edata"
						for (int i = 0; i < numberOfSections; i++) {
							...
							if (sectionName.equals(".edata")) {
								...
								// we found it, no need to loop over more
								// sections
								break;
								...
								// NOTE: the spec says this is from the Optional
								// Header,
								// that is incorrect, this is from the start of
								// the file
								...
								// and skip the last 16 bytes of the section
								// header
						...
						if (!foundEDataSectionHeader) {
							...
							// NOTE: (as above) gives an actually offset
						...
						for (int i = 0; i < numNamePointers; i++) {
							...
							// if the function's RVA is inside the export
							// section, is is a forwarded
						...
						if (inputFile != null) {
							try {
								...
								// don't want to re-throw because we are already
								// in a finally block
							}
						}
					}
	...
	String generateDllVersionInfoString() {
		if (0 == timeDateStamp) {
			...
			return "DLL Versioning Info:\r" + "TimeDateStamp: " + timeDateStamp + "\r" + "OperatingSystemVersion: "
					+ majorOperatingSystemVersion + "." + minorOperatingSystemVersion + "\r" + "ImageVersion: "
					+ majorImageBVersion + "." + minorImageVersion + "\r" + "SubsystemVersion: " + majorSubsystemVersion
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setImage(
				ImageDescriptor.createFromURL(getClass().getResource("/icons/load_library_exports.gif")).createImage());
	}
	...
	protected void okPressed() {
		...
		if (functionsList.size() > 0) {
			...
			setChannelTypeFuncSet(functionsList, dllFileName);
			FunctionsView functionsView = (FunctionsView) PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getActivePage().findView(FunctionsView.ID);
			if (functionsView != null) {
				functionsView.refresh();
			}

			ThreadFunctionsView threadFunctionsView = (ThreadFunctionsView) PlatformUI.getWorkbench()
					.getActiveWorkbenchWindow().getActivePage().findView(ThreadFunctionsView.ID);
	...
	private void setChannelTypeFuncSet(List<Pair<Long, String>> functionsList, String dllFileName) {
		IWorkbenchWindow activeWorkbenchWindow = PlatformUI.getWorkbench().getActiveWorkbenchWindow();
		AtlantisTraceEditor activeTraceDisplayer = (AtlantisTraceEditor) activeWorkbenchWindow.getActivePage()
				.getActiveEditor();
		AtlantisFileModelDataLayer fileModel = (AtlantisFileModelDataLayer) RegistryUtils
		...
		try (final FileReader fileReader = new FileReader(jsonFile.getAbsolutePath())) {
			...
			for (ChannelType channelType : channelTypeList) {
				...
				for (Pair<Long, String> function : functionsList) {
					for (ChannelFunction channelFunction : channelType.getChannelOpenStageList()) {
						if (channelFunction.getFunctionName().equals(function.getRight())) {
							Function func = fileModel.getFunctionDb().getSingleFunctionFromModule(dllFileName,
									function.getLeft(), fileModel.getInstructionDb());
							if (func != null) {
								channelOpenStageList
										.add(new ChannelFunctionInstruction(channelFunction,
												fileModel
														.getFunctionDb().getSingleFunctionFromModule(dllFileName,
																function.getLeft(), fileModel.getInstructionDb())
														.getFirst()));
							}
						}
				...
				if (channelOpenStageList.size() != 0) {
					activeTraceDisplayer.addAllToChannelOpenStageList(channelType.getChannelTypeName(), channelOpenStageList);
				}
				if (dataTransStageList.size() != 0) {
					activeTraceDisplayer.addAllToDataTransStageList(channelType.getChannelTypeName(), dataTransStageList);
				}
				if (channelCloseStageList.size() != 0) {
					activeTraceDisplayer.addAllToChannelCloseStageList(channelType.getChannelTypeName(), channelCloseStageList);
				}
			}
		} catch (IOException e) {

...
import ca.uvic.chisel.atlantis.database.DiffResultsDbConnection;
...
public class TextMergeViewer extends ContentMergeViewer implements IAdaptable {
	...
	protected boolean fHighlightRanges;
	...
	// SWT widgets
	protected BufferedCanvas fAncestorCanvas;
	protected BufferedCanvas fLeftCanvas;
	protected BufferedCanvas fRightCanvas;
	protected BufferedCanvas fCenterCanvas;
	protected BufferedCanvas fCenterDiffCanvas;
	protected Canvas fScrollCanvas;
	protected ScrollBar fVScrollBar;
	protected Canvas fBirdsEyeCanvas;
	protected Canvas fSummaryHeader;
	protected HeaderPainter fHeaderPainter;
	...
	private DiffRegion fButtonDiff;
	...
	private ArrayList<DiffRegion> allMatchingRegions;
	private ArrayList<DiffRegion> allDifferentRegions;
	private ArrayList<DiffRegion> allRegions;
	private ArrayList<ArrayList<DiffRegion>> results = null;
	private DiffRegion currentDiffRegion = null;
	
	private boolean delaySyntaxHighlighting = false;
	
	public BigFileViewer fLeftTraceViewer;
	public BigFileViewer fRightTraceViewer;
	public BigFileViewer fAncestorTraceViewer;
	
	public Date lastEventTime = Calendar.getInstance().getTime();
	public Date currentEventTime = null;
...
//	private DocumentMerger fMerger;
	...
	private final class InternalOutlineViewerCreator extends OutlineViewerCreator implements ISelectionChangedListener {
		...
		public void selectionChanged(SelectionChangedEvent event) {
			...
			if (s instanceof IStructuredSelection) {
				IStructuredSelection ss = (IStructuredSelection) s;
				Object element = ss.getFirstElement();
				DiffRegion diff = findDiff(element);
				if (diff != null)
					setCurrentDiff(diff, true);
			}
		}
	...
	/*	private Diff findDiff(Object element) {
		...
		private DiffRegion findDiff(Object element) {
			if (element instanceof ICompareInput) {
				ICompareInput ci = (ICompareInput) element;
				Position p = getPosition(ci.getLeft());
				if (p != null)
	 ...
	 /*   private Diff findDiff(Position p, boolean left) {
	    ...
	    private DiffRegion findDiff(Position p, boolean left) {
			for (Iterator iterator = allRegions.iterator(); iterator.hasNext();) {
				DiffRegion diff = (DiffRegion) iterator.next();
				Position diffPos;
				if (left) {
					diffPos = new Position((int)diff.getLeftOffset(), diff.getLeftLength());
				} else {
					diffPos = new Position((int)diff.getRightOffset(), diff.getRightLength());
				}
				// If the element falls within a diff, highlight that diff
	...
	class ContributorInfo implements IElementStateListener, VerifyListener, IDocumentListener, IEncodingSupport {
		...
		public void setDocument(MergeSourceViewer viewer, boolean isEditable, char type) {
			// Ensure that this method is only called once
			Assert.isTrue(fSourceViewer == null);
			fSourceViewer = viewer;
			try {
				internalSetDocument(viewer, type);
		 ...
		 */
		private boolean internalSetDocument(MergeSourceViewer tp, char type) {
				...
				if (newDocument == null) {
					...
					/*switch(type){
						case LEFT_CONTRIBUTOR:
							fLeftTraceViewer.setFileModel((FileEditorInput)fLeftContributor.getDocumentKey(),fLeftTraceViewer);
							fLeft.getSourceViewer().setDocument(fLeftTraceViewer.getDocument(),fLeftTraceViewer.getVisualAnnotationModel());
							break;
						case RIGHT_CONTRIBUTOR:
							fRightTraceViewer.setFileModel((FileEditorInput)fRightContributor.getDocumentKey(),fRightTraceViewer);
							fRight.getSourceViewer().setDocument(fRightTraceViewer.getDocument(),fRightTraceViewer.getVisualAnnotationModel());
							break;
						case ANCESTOR_CONTRIBUTOR:
							fAncestorTraceViewer.setFileModel((FileEditorInput)fRightContributor.getDocumentKey(),fRightTraceViewer);
							fAncestor.getSourceViewer().setDocument(fAncestorTraceViewer.getDocument(),fAncestorTraceViewer.getVisualAnnotationModel());
							break;
					}*/
			...
			if (newDocument != oldDoc) {
				updateViewerDocument(tp, newDocument, range, type);
		 ...
		 */
		private void updateViewerDocument(MergeSourceViewer tp, IDocument document, Position range, char type) {
			unsetDocument(tp);
			if (document == null)
				return;
			...
			// Modified to support code folding in TraceCompareViewer (currently unused, however)
		//	IAnnotationModel annotationModel = new AnnotationModel();
			IAnnotationModel somethingModel = null;
			switch(type){
				case LEFT_CONTRIBUTOR:
					somethingModel = fLeftTraceViewer.getAnnotationModel();
					break;
				case RIGHT_CONTRIBUTOR:
					somethingModel = fRightTraceViewer.getAnnotationModel();
					break;			
				case ANCESTOR_CONTRIBUTOR:
					somethingModel = fAncestorTraceViewer.getAnnotationModel();
					break;
			}
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					...
					//tp.getSourceViewer().setDocument(document, annotationModel, r.getOffset(), r.getLength());
					tp.getSourceViewer().setDocument(document, somethingModel, r.getOffset(), r.getLength());
				} else
					//tp.getSourceViewer().setDocument(document, annotationModel);
					tp.getSourceViewer().setDocument(document, somethingModel);
			} else
				//tp.getSourceViewer().setDocument(document, annotationModel);
				tp.getSourceViewer().setDocument(document, somethingModel);
	...
	/*	public void applyTextPresentation(TextPresentation textPresentation) {
		...
		public void applyTextPresentation(TextPresentation textPresentation) {
			if (!fHighlightTokenChanges)
				return;
			IRegion region= textPresentation.getExtent();		
			if(allDifferentRegions != null){
				for(Iterator iterator = allDifferentRegions.iterator(); iterator.hasNext();){
					DiffRegion diff = (DiffRegion) iterator.next();
					StyleRange range = getStyleRange(diff, region);
					if (range != null)
						textPresentation.mergeStyleRange(range);
...
/*		private StyleRange getStyleRange(Diff diff, IRegion region) {
		...
		private StyleRange getStyleRange(DiffRegion diff, IRegion region) {
			//Color cText = getColor(null, getTextColor());
			Color cTextFill = getColor(null, getTextFillColor(diff));
			if (cTextFill == null)
				return null;
			char contrib = getLeg(viewer);
			Position p = null;
			switch(contrib){
			case(LEFT_CONTRIBUTOR):
				p = new Position((int)diff.getLeftOffset(), diff.getLeftLength());
			case(RIGHT_CONTRIBUTOR):
				p = new Position((int)diff.getRightOffset(), diff.getRightLength());
			}
			int start = p.getOffset();
			int length = p.getLength();
			// Don't start before the region
	...
	/*	private RGB getTextFillColor(Diff diff) {
		...
		private RGB getTextFillColor(DiffRegion diff) {
			return OUTGOING_TEXT_FILL;
		}
	...
	// Changed to protected from private to support TraceCompareViewer's need to refresh the centre control
	...
	public IEditorInput getContributor(char type){
		switch(type){
			case ANCESTOR_CONTRIBUTOR:
	...
	protected void createControls(Composite composite) {
		...
		fAncestor= createPart(composite, 'A');
	//	fAncestor.getSourceViewer().setDocument(fAncestorTraceViewer.getDocument(),fAncestorTraceViewer.getVisualAnnotationModel());
	...
	//	fRight.getSourceViewer().setDocument(fRightTraceViewer.getDocument(),fRightTraceViewer.getVisualAnnotationModel());
		...
		fLeftTraceViewer.getTextWidget().addMouseWheelListener(new MouseWheelListener(){
			@Override
			public void mouseScrolled(MouseEvent e){
				currentEventTime = Calendar.getInstance().getTime();
			//	delaySyntaxHighlighting = true;
				syncTextWidget();
			}
		});
		
		fRightTraceViewer.getTextWidget().addMouseWheelListener(new MouseWheelListener(){
			@Override
			public void mouseScrolled(MouseEvent e){
				currentEventTime = Calendar.getInstance().getTime();
			//	delaySyntaxHighlighting = true;
		...
		fLeftTraceViewer.getTextWidget().addCaretListener(new CaretListener() {
			@Override
			public void caretMoved(CaretEvent event) {
		//		currentEventTime = Calendar.getInstance().getTime();
		//		delaySyntaxHighlighting = false;
					syncTextWidget();
				}
		});
		
		fRightTraceViewer.getTextWidget().addCaretListener(new CaretListener() {
			@Override
			public void caretMoved(CaretEvent event) {
		//		currentEventTime = Calendar.getInstance().getTime();
			//	delaySyntaxHighlighting = false;
		...
		fLeftTraceViewer.getTextWidget().addKeyListener(new KeyListener() {
			@Override
			...
			public void keyReleased(KeyEvent e) {
				if(e.keyCode == SWT.ARROW_DOWN || e.keyCode == SWT.ARROW_UP || e.keyCode == SWT.PAGE_DOWN || 
					...
					//fLeftTraceViewer.getSyntaxHighlightingManager().setHighlightingDirty();
			//		currentEventTime = Calendar.getInstance().getTime();
					delaySyntaxHighlighting = false;
					syncTextWidget();
				}
		...
		fRightTraceViewer.getTextWidget().addKeyListener(new KeyListener() {
			@Override
			public void keyPressed(KeyEvent e) {
				delaySyntaxHighlighting = true;
				return;
			...
			public void keyReleased(KeyEvent e) {
				if(e.keyCode == SWT.ARROW_DOWN || e.keyCode == SWT.ARROW_UP || e.keyCode == SWT.PAGE_DOWN || 
						e.keyCode == SWT.PAGE_UP || e.keyCode == SWT.HOME || e.keyCode == SWT.END) {
					//fRightTraceViewer.getSyntaxHighlightingManager().setHighlightingDirty();
		//			currentEventTime = Calendar.getInstance().getTime();
					delaySyntaxHighlighting = false;
					syncTextWidget();
				}
		 ...
		 if (part != null) {
			...
			//	 cesp.setViewers(new SourceViewer[] {fLeftTraceViewer, fRightTraceViewer,fAncestorTraceViewer},focusSourceViewer);
			 }
		 }
		...
		fBirdsEyeCanvas.addMouseMoveListener(
			new MouseMoveListener() {
				...
				public void mouseMove(MouseEvent e) {
					Cursor cursor= null;
					DiffRegion diff= handlemouseInBirdsEyeView(fBirdsEyeCanvas, e.y);
					if (diff != null)
	...
	public void syncTextWidget(){
		int startIndex = fRight.getSourceViewer().getTextWidget().getTopIndex();
		int bottomIndex = Math.max(startIndex + (fRight.getSourceViewer().getTextWidget().getClientArea().height / fRight.getSourceViewer().getTextWidget().getLineHeight()), fRight.getSourceViewer().getBottomIndex());
		if(fFocusPart != null){
			startIndex = fFocusPart.getSourceViewer().getTextWidget().getTopIndex();
			bottomIndex = Math.max(startIndex + (fFocusPart.getSourceViewer().getTextWidget().getClientArea().height / fFocusPart.getSourceViewer().getTextWidget().getLineHeight()), fFocusPart.getSourceViewer().getBottomIndex());
		}
	/*	if(delaySyntaxHighlighting == true){
			if(lastEventTime != null && currentEventTime != null){
				if(currentEventTime.getTime() - lastEventTime.getTime() > 300){
					delaySyntaxHighlighting = false;
				}			
			}
		}*/
		if(currentEventTime != null){
			lastEventTime.setTime(currentEventTime.getTime());
		}
		
		if(fLeftTraceViewer.isLoaded()){
			fLeftTraceViewer.synchronizedUpdateTextWidget(startIndex, bottomIndex, delaySyntaxHighlighting);
			fLeft.getSourceViewer().getTextWidget().redraw();
		}		
		if(fRightTraceViewer.isLoaded()){
			fRightTraceViewer.synchronizedUpdateTextWidget(startIndex, bottomIndex, delaySyntaxHighlighting);
	...
	private void setCurrentDiff2(DiffRegion diff, boolean reveal) {
		if (diff != null) {
			//fCurrentDiff= null;
...
/*	private Diff handleMouseInSides(Canvas canvas, MergeSourceViewer tp, int my) {
	...
	private DiffRegion handleMouseInSides(Canvas canvas, MergeSourceViewer tp, int my) {
		...
		if (allDifferentRegions != null) {
			...
			for (Iterator iterator = allDifferentRegions.iterator(); iterator.hasNext();) {
				...
				if (fShowCurrentOnly2 && !isCurrentDiff(diff))
					continue;
				if(leg == LEFT_CONTRIBUTOR){
					region = new Point(diff.getLeftStartLine() - 1, diff.getLeftRegionHeight());
				}
				else{
					region = new Point(diff.getRightStartLine() - 1, diff.getRightRegionHeight());
				}
		//		tp.getLineRange(diff.getPosition(leg), region);
				int y= (region.x * lineHeight) + shift;
				int h= region.y * lineHeight;
	...
	private DiffRegion getDiffUnderMouse(Canvas canvas, int mx, int my, Rectangle r) {
		...
		if (allDifferentRegions != null) {
			...
			//	fLeft.getLineRange(new Position((int)diff.getLeftOffset(), diff.getLeftLength()), region);
				region = new Point(diff.getLeftStartLine() - 1, diff.getLeftRegionHeight());
				int ly= (region.x * lineHeight) + lshift;
				int lh= region.y * lineHeight;

			//	fRight.getLineRange(new Position((int)diff.getRightOffset(), diff.getRightLength()), region);
				region = new Point(diff.getLeftStartLine() - 1, diff.getRightRegionHeight());
				int ry= (region.x * lineHeight) + rshift;
				int rh= region.y * lineHeight;
...
/*	private Diff handlemouseInBirdsEyeView(Canvas canvas, int my) {
	...
	private DiffRegion handlemouseInBirdsEyeView(Canvas canvas, int my) {
		Point p = canvas.getSize();
		return findDiff(p.x, p.x + p.y);
	//	return fMerger.findDiff(getViewportHeight(), fSynchronizedScrolling, canvas.getSize(), my);
	}
	...
	*/
	...
	private void paintBirdsEyeView(Canvas canvas, GC gc) {
		...
		Display display= canvas.getDisplay();
		int y= 0;
		for (Iterator iterator = allRegions.listIterator(); iterator.hasNext();) {
			DiffRegion diff = (DiffRegion) iterator.next();
			int h= diff.getMaxRegionHeight();
							
	//		if (fMerger.useChange(diff)) {
				
		//		yy= (y*size.y)/virtualHeight;
		//		hh= (h*size.y)/virtualHeight;
			yy = (diff.getLeftStartLine()*size.y)/virtualHeight;
			hh = (h*size.y)/virtualHeight;
				if (hh < 3)
					hh= 3;
				
			//	c= getColor(display, getFillColor(diff));
				c = getColor(display, new RGB(255,0,0));
		//		if (c != null) {
				if(diff.getMatchState() == false){
					gc.setBackground(c);
					gc.fillRectangle(BIRDS_EYE_VIEW_INSET, yy, size.x-(2*BIRDS_EYE_VIEW_INSET),hh);
				}
				c= getColor(display, getStrokeColor(diff));
...
/*	private void paintBirdsEyeView(Canvas canvas, GC gc) {
	...
	protected final Control createCenterControl(Composite parent) {
		if (fSynchronizedScrolling) {
			fCenterCanvas= new BufferedCanvas(parent, SWT.NONE) {
				@Override
				public void doPaint(GC gc) {
					paintCenter(this, gc);
				}
			...
			if (fUseResolveUI) {
				...
				new HoverResizer(fCenterCanvas, HORIZONTAL);
								
				fCenterButton= new Button(fCenterCanvas, fIsMac ? SWT.FLAT : SWT.PUSH);
				if (fNormalCursor == null) fNormalCursor= new Cursor(fCenterCanvas.getDisplay(), SWT.CURSOR_ARROW);
				fCenterButton.setCursor(fNormalCursor);
				fCenterButton.setText(COPY_RIGHT_TO_LEFT_INDICATOR);
				fCenterButton.pack();
				fCenterButton.setVisible(false);
				...
				new Resizer(fCenterCanvas, HORIZONTAL);
			}
			
			return fCenterCanvas;
		}
		return super.createCenterControl(parent);
	}
...
/*	private boolean handleMouseMoveOverCenter(Canvas canvas, int x, int y) {
	...
	private boolean handleMouseMoveOverCenter(Canvas canvas, int x, int y) {
		Rectangle r= new Rectangle(0, 0, 0, 0);
		DiffRegion diff= getDiffUnderMouse(canvas, x, y, r);
		if (diff != fButtonDiff) {
			if (diff != null) {
				if (fLeft.getSourceViewer().isEditable()) {
					fButtonDiff= diff;
	 ...
	 */
	private MergeSourceViewer createPart(Composite parent, char type) {
		BigFileViewer traceViewer = (BigFileViewer)createSourceViewer(parent, getDirection(), type);
		switch(type){
			case ANCESTOR_CONTRIBUTOR:
				fAncestorTraceViewer = traceViewer;
				break;
			case LEFT_CONTRIBUTOR:
				fLeftTraceViewer = traceViewer;
				break;
			case RIGHT_CONTRIBUTOR:
				fRightTraceViewer = traceViewer;
				break;
		}
					...
					public void lineGetBackground(LineBackgroundEvent event) {
						...
						if (textWidget != null) {
							...
							if (event.lineOffset <= caret
								...
								DiffRegion diff = findDiff(event.lineOffset,
	...
	protected void updateContent(Object ancestor, Object left, Object right) {
		...
		// set new documents
		fLeftContributor.setDocument(fLeft, cc.isLeftEditable() && cp.isLeftEditable(input), LEFT_CONTRIBUTOR);
		fLeftTraceViewer.setDiffFileModel((FileEditorInput)fLeftContributor.getDocumentKey(), fLeftTraceViewer);
		fLeftTraceViewer.init((FileEditorInput)fLeftContributor.getDocumentKey(), fLeftTraceViewer);
		fLeftLineCount= fLeft.getLineCount();
		
		
		fRightContributor.setDocument(fRight, cc.isRightEditable() && cp.isRightEditable(input), RIGHT_CONTRIBUTOR);
		fRightTraceViewer.setDiffFileModel((FileEditorInput)fRightContributor.getDocumentKey(), fRightTraceViewer);
		fRightTraceViewer.init((FileEditorInput)fRightContributor.getDocumentKey(), fRightTraceViewer);
		fRightLineCount= fRight.getLineCount();
		
		fAncestorContributor.setDocument(fAncestor, false, ANCESTOR_CONTRIBUTOR);
	...
	/*					if (leftRange != null)
						...
						if (leftRange != null){
							int leftStart = leftRange.offset;
							int leftEnd = leftStart + leftRange.length;
							selectDiff = findDiff(leftStart, leftEnd);
						 //   selectDiff= fMerger.findDiff(LEFT_CONTRIBUTOR, leftRange);
						}
						else if (rightRange != null){
							int rightStart = rightRange.offset;
							int rightEnd = rightStart + rightRange.length;
							selectDiff = findDiff(rightStart, rightEnd);
						   // selectDiff= fMerger.findDiff(RIGHT_CONTRIBUTOR, rightRange);
						}
					}
					if (selectDiff != null)
	...
	private boolean updateDiffBackground(DiffRegion diff) {
...
/*		if (other instanceof IDocumentRange) {
	...
	private void handleSelectionChanged(MergeSourceViewer tw) {
		...
		DiffRegion d= findDiff(p.x, p.x+p.y);
		updateStatus(d);
	...
	public BigFileViewer getLeftTraceViewer(){
		return fLeftTraceViewer;
	}
	
	public BigFileViewer getRightTraceViewer(){
		return fRightTraceViewer;
	}
	//---- the differencing
	
	// Modified for TraceCompareViewer - changed to protected from private
	
	protected int getNumberOfConsumers(){
		// XXX FIXME
		System.out.println("Diffing will be broken here due to changes in DB table naming.");
		String challengerTableName = fRightContributor.getDocumentKey().getName().replace("trace", "original");
		...
		try {
			...
			challengerDB = null; // new TraceFileLineDbConnection(challengerTableName, null);
			int challengerSize = challengerDB.getNumberOfLines();
			int numberOfConsumers = challengerSize/400;
	...
	protected void doDiff() {
		String referenceTableName = fLeftContributor.getDocumentKey().getName().replace(AtlantisFileUtils.BINARY_FORMAT_TMP_FILE_EXTENSION, "");
		String challengerTableName = fRightContributor.getDocumentKey().getName().replace(AtlantisFileUtils.BINARY_FORMAT_TMP_FILE_EXTENSION, "");

		try {
	//		TraceFileLineDbConnection referenceDB = new TraceFileLineDbConnection(referenceTableName);
	//		TraceFileLineDbConnection challengerDB = new TraceFileLineDbConnection(challengerTableName);
			// TODO I needed to get the actual file reference for use with individualized database file per trace.
			// I had to comment this out. The diffs are not production ready at this time,
			// and should be modified to make use of the function call data now available anyway.
			System.out.println("Disabled TextMergeView");
			DiffResultsDbConnection diffResultsTable = null; //= new DiffResultsDbConnection(referenceTableName, null, challengerTableName, null);
			if(diffResultsTable.createTables()){
	//			int challengerSize = challengerDB.getNumberOfLines();
	//			int numberOfConsumers = challengerSize/400;		
				IRunnableWithProgress runnable= new IRunnableWithProgress() {
					@Override
					public void run(IProgressMonitor monitor) throws InterruptedException, InvocationTargetException {
						monitor.beginTask(CompareMessages.DocumentMerger_0, getNumberOfConsumers());
						try {
							String referenceTableName = fLeftContributor.getDocumentKey().getName().replace("trace", "original");
							String challengerTableName = fRightContributor.getDocumentKey().getName().replace("trace", "original");
							int numberOfConsumers = getNumberOfConsumers();
							SimpleSQLDiff SQLDiff = new SimpleSQLDiff(referenceTableName, null, challengerTableName, null, numberOfConsumers);
							results = SQLDiff.doTheDiff(monitor);
				...
				allMatchingRegions = results.get(0);
				allDifferentRegions = results.get(1);
				allRegions = results.get(2);
				diffResultsTable.saveDiffResults(allRegions);
			}else{
				allMatchingRegions = diffResultsTable.getMatchingRegions();
				allDifferentRegions = diffResultsTable.getDifferentRegions();
				allRegions = diffResultsTable.getAllRegions();
			}
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
...
/*	private Diff findDiff(char type, int pos) {
	...
	private void updateControls() {
		...
		updateStatus(currentDiffRegion);
		updateResolveStatus();
	...
	private void updateStatus(DiffRegion diff) {
		...
		//		diff= diff.getParent();
			...
			diffDescription= MessageFormat.format(format,
				new String[] {
					getDiffType(),						// 0: diff type
					getDiffNumber(diff),					// 1: diff number
					getDiffRange(fLeft, new Position((int)diff.getLeftOffset(), diff.getLeftLength())),		// 2: left start line
					getDiffRange(fRight, new Position((int)diff.getRightOffset(), diff.getRightLength()))	// 3: left end line
				}
			);
		}
		...
		String s= MessageFormat.format(format,
			new String[] {
				getCursorPosition(fLeftTraceViewer),	// 0: left column
				getCursorPosition(fRightTraceViewer),	// 1: right column
	...
	private String getDiffType() {
		...
		s= CompareMessages.TextMergeViewer_direction_outgoing;
		String format= CompareMessages.TextMergeViewer_diffType_format;
		return MessageFormat.format(format, new String[] { s, CompareMessages.TextMergeViewer_changeType_addition } );
	}
	...
	private String getDiffNumber(DiffRegion diff) {
		// find the diff's number
		int diffNumber= 0;
		if (allDifferentRegions != null) {
			diffNumber = allDifferentRegions.indexOf(diff);
		}
		return Integer.toString(diffNumber + 1);
	}
	 ...
	 */
	private String getCursorPosition(BigFileViewer v) {
		if (v != null) {
...
//			invalidateLines();
	...
	*/
	...
	private void paintCenter(Canvas canvas, GC g) {
		...
		//		Color fillColor= getColor(display, getFillColor(diff));
				Color fillColor = getColor(display, new RGB(255,0,0));
				Color strokeColor= getColor(display, getStrokeColor(diff));
				...
				if (fUseSingleLine && showResolveUI && false) {
					// draw resolve state
					int cx= (w-RESOLVE_SIZE)/2;
					int cy= ((ly+lh/2) + (ry+rh/2) - RESOLVE_SIZE)/2;
	...
	//			tp.getLineRange(diff.getPosition(leg), region);
			...
			//	g.setBackground(getColor(display, getFillColor(diff)));
				g.setBackground(getColor(display, new RGB(255,0,0)));
				if (right)
					g.fillRectangle(x, y, w2, h);
				else
					g.fillRectangle(x+w2, y, w2, h);
...
*/
	private void paint(PaintEvent event, MergeSourceViewer tp) {
		...
		for (Iterator iterator = allDifferentRegions.iterator(); iterator.hasNext();) {
			...
			if(leg == LEFT_CONTRIBUTOR){
				range = new Point(diff.getLeftStartLine() - 1, diff.getLeftRegionHeight());
			}else{
				range = new Point(diff.getRightStartLine() - 1, diff.getRightRegionHeight());
			}
	...
	private RGB getFillColor(DiffRegion diff) {
		boolean selected = currentDiffRegion != null;
	...
	private RGB getStrokeColor(DiffRegion diff) {
	...
	private DiffRegion getNextVisibleDiff(boolean down, boolean deep) {
		DiffRegion diff= null;
		MergeSourceViewer part= getNavigationPart();
		if (part == null)
			return null;
		Point s = part.getSourceViewer().getSelectedRange();
	...
	//	for (;;) {
			diff = null;
			diff = internalGetNextDiff(down, deep, part, s);
		//	if (diff != null && !isAncestorVisible()) {
		//		Position position = null;
		//		if(leg == LEFT_CONTRIBUTOR){			
		//			position = new Position((int)diff.getLeftOffset(), diff.getLeftLength());
		//		}
		//		else{
		//			position = new Position((int)diff.getRightOffset(), diff.getRightLength());
		//		}
		//		Position position = diff.getPosition(leg);
		//		s = new Point(position.getOffset(), position.getLength());
		//		diff= null;
	...
	private DiffRegion internalGetNextDiff(boolean down, boolean deep, MergeSourceViewer part, Point s) {
		if (allDifferentRegions != null) {
			if (down)
				return findNext();
			return findPrev();
	...
	private DiffRegion getWrappedDiff(DiffRegion diff, boolean down) {
		if (allDifferentRegions != null && allDifferentRegions.size() > 0) {
			if (down)
				return (DiffRegion) allDifferentRegions.get(0);
			return (DiffRegion) allDifferentRegions.get(allDifferentRegions.size()-1);
		}
		return null;
	}
...
/*	private boolean navigate(boolean down, boolean wrap, boolean deep) {
	...
	private DiffRegion findDiff(int rangeStart, int rangeEnd) {
		for(Iterator iterator = allDifferentRegions.iterator(); iterator.hasNext();){
			DiffRegion diff = (DiffRegion) iterator.next();
			if(diff.getLeftOffset() >= rangeStart && (diff.getLeftOffset() + diff.getLeftLength()) <= rangeEnd){
				return diff;
			}
		}
		return null;
	...
	private DiffRegion findNext() {
		if(allDifferentRegions.size() == 0){
			return null;
		}
		if(currentDiffRegion == null){
			return allDifferentRegions.get(0);
		}else{
			int currentIndex = allDifferentRegions.indexOf(currentDiffRegion);
			DiffRegion newCurrentDiffRegion = ((currentIndex + 1) < allDifferentRegions.size()) ? allDifferentRegions.get(currentIndex + 1) : allDifferentRegions.get(allDifferentRegions.size() -1);
			if(newCurrentDiffRegion != null){
	//			currentDiffRegion = newCurrentDiffRegion;
				return newCurrentDiffRegion;
			}else{
				return currentDiffRegion;
			}
		}
	}
/*	private DiffRegion findNext(MergeSourceViewer tp, int start, int end, boolean deep) {
		return fMerger.findNext(getLeg(tp), start, end, deep);
	}*/
	
	private DiffRegion findPrev() {
		if(allDifferentRegions.size() == 0){
			return null;
		}
		if(currentDiffRegion == null){
			return allDifferentRegions.get(allDifferentRegions.size() -1);
		}else{
			int currentIndex = allDifferentRegions.indexOf(currentDiffRegion);
			DiffRegion newDiffRegion = ((currentIndex -1) <= 0) ? allDifferentRegions.get(0) : allDifferentRegions.get(currentIndex - 1);
			if(newDiffRegion != null){
				return newDiffRegion;
			}else{
				return currentDiffRegion;
			}
		}
	...
	private void setCurrentDiff(DiffRegion d, boolean revealAndSelect) {
		setCurrentDiff(d, revealAndSelect, false);
	}
	/*
	 * Set the currently active Diff and update the toolbars controls and lines.
...
//			return;
	...
	private void setCurrentDiff(DiffRegion d, boolean revealAndSelect, boolean deep) {

//		if (d == fCurrentDiff)
//			return;
		if(d == currentDiffRegion)
			return;
		boolean diffChanged = currentDiffRegion != d;
		...
		//	fLeft.getSourceViewer().getTextWidget().setSelection((int)d.getLeftOffset());
		//	fRight.getSourceViewer().getTextWidget().setSelection((int)d.getRightOffset());
			fLeft.setSelection(new Position((int)d.getLeftOffset(), d.getLeftLength()));
			fRight.setSelection(new Position((int)d.getRightOffset(), d.getRightLength()));

	//		fLeftTraceViewer.setSelectedRange((int)d.getLeftOffset(), d.getLeftLength());
	//		fRightTraceViewer.setSelectedRange((int)d.getRightOffset(), d.getRightLength());

		//	fLeft.getSourceViewer().getTextWidget().setCaretOffset((int)d.getLeftOffset());
		//	fRight.getSourceViewer().getTextWidget().setCaretOffset((int)d.getRightOffset());
...
/*	private void revealDiff(Diff d, boolean smart) {
	...
	private void revealDiff(DiffRegion d, boolean smart) {
		...
		if (smart) {
			Point region= new Point(0, 0);
			// find the starting line of the diff in all text widgets
	//		int ls= fLeft.getLineRange(new Position((int)d.getLeftOffset(), d.getLeftLength()), region).x;
			int ls = new Point(d.getLeftStartLine() - 1, d.getLeftRegionHeight()).x;
	//		int rs= fRight.getLineRange(new Position((int)d.getRightOffset(), d.getRightLength()), region).x;
			int rs = new Point(d.getRightStartLine() - 1, d.getRightRegionHeight()).x;
	...
	private int getVirtualHeight(){
		int h= 1;
		if (allRegions != null) {
			Iterator e= allRegions.iterator();
			while (e.hasNext()) {
				DiffRegion diff= (DiffRegion) e.next();
				//h+= diff.getMaxRegionHeight();
				h += diff.getLeftRegionHeight();
			}
		}
		h = Math.max(fLeftTraceViewer.getTextWidget().getLineCount(), fRightTraceViewer.getTextWidget().getLineCount());
		return h;
	}
...
/*	private int virtualToRealPosition(char contributor, int v) {
	...
	private void resetDiffs() {
		...
		fCurrentDiff= null;
		allRegions = null;
		allDifferentRegions = null;
		allMatchingRegions = null;
		resetPositions(fLeft.getSourceViewer().getDocument());
		resetPositions(fRight.getSourceViewer().getDocument());
		resetPositions(fAncestor.getSourceViewer().getDocument());
	}
	...
	private boolean isCurrentDiff(DiffRegion diff) {
		if (diff == null)
			return false;
		if (diff == currentDiffRegion)
			return true;
		return false;
	}
	...
	private boolean isNavigationPossible() {
		if (currentDiffRegion == null && allDifferentRegions != null)
			return true;
		else if (allDifferentRegions.size() > 1)
			return true;
		else if (fCurrentDiff != null && fCurrentDiff.hasChildren())
			return true;
		else if (fCurrentDiff != null && fCurrentDiff.isToken())
	...
	// Extra methods to support TraceCompareViewer functionality

	/**
	 * Gets the source viewer for the left document.
	 * @return left source viewer
	 */
	protected SourceViewer getLeftSourceViewer() {
		return this.fLeft.getSourceViewer();
	}
	
	/**
	 * Gets the source view for the right document.
	 * @return right source viewer
	 */
	protected SourceViewer getRightSourceViewer() {
		return this.fRight.getSourceViewer();
	}
	
	/**
	 * Gets diff results that include both matching sections and sections that differ.
	 * @return list of Diff objects that represent all diff results
	 */
	/*protected List<Diff> getAllResults() {
		Iterator<Diff> iterator = fMerger.rangesIterator();
		List<Diff> allResults = new ArrayList<Diff>();
	...
	protected List<DiffRegion> getAllResults() {
		return allRegions;
	}
	
	/**
	 * Gets diff results that only include the sections that differ.
	 * @return list of Diff objects representing differing sections
	 */
/*	protected List<Diff> getDifferences() {
		Iterator<Diff> iterator = fMerger.changesIterator();
	...
	protected List<DiffRegion> getDifferences() {
		return allDifferentRegions;
	}
	
	
	protected List<DiffRegion> getMatches(){
		return allMatchingRegions;
	}
	
	protected IEditorInput getLeftContributorKey(){
		return fLeftContributor.getDocumentKey();
	}
	
	protected IEditorInput getRightContributorKey(){
		return fRightContributor.getDocumentKey();
	}
	
	protected void setHighlightRanges(boolean state){
		fHighlightRanges = state;
	}

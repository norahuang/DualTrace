...
public class DbFileSearchQuery extends FileSearchQuery {
//implements ISearchQuery {
	...
	public DbFileSearchQuery(IFileModelDataLayer fileModel, String searchText, boolean isRegEx, boolean isCaseSensitive, FileTextSearchScope scope) {
	...
	public IStatus run(final IProgressMonitor monitor) {
		...
		DbTextSearchResultCollector collector= new DbTextSearchResultCollector(textResult, isFileNameSearch(), searchInBinaries);
	...
	public IStatus searchInFile(final AbstractTextSearchResult result, final IProgressMonitor parentRunMonitor, IFile file) {
		...
		Pattern searchPattern= getSearchPattern();
		DbTextSearchResultCollector collector= new DbTextSearchResultCollector(result, isFileNameSearch(), true);
	...
	private TextSearchEngine createDefault(final IFileModelDataLayer fileModel, IFile file, final boolean useRegexSearch) {
		...
		return new TextSearchEngine() {
			...
			public IStatus search(TextSearchScope scope, TextSearchRequestor requestor, Pattern searchPattern, IProgressMonitor parentRunMonitor) {
				return new DbTextSearchVisitor(requestor, searchPattern, useRegexSearch).search(scope, parentRunMonitor, fileModel);
			}

 ...
 * @author Laura Chan
 */
public class RenameCommentGroupDialog extends TitleAreaDialog {
	 ...
	 */
	public RenameCommentGroupDialog(Shell parentShell, String oldName) {
		super(parentShell);
	...
	public void create() {
		super.create();
		this.setTitle("Rename comment group");
		this.setMessage("Enter a new name for this comment group.");
	}
			...
			public void handleEvent(Event event) {
				if (isValidName()) {
					...
					if (nameFieldEmpty()) {
						...
						setMessage("There is already an existing comment group with that name. Please choose another name.", IMessageProvider.ERROR);
					}
				}
			}
		});
	...
	private boolean isValidName() {
		...
		return editor.getProjectionViewer().getFileModel().isUniqueCommentGroupName(nameField.getText().trim());
	}

...
public class IntervalTree<Type> {
	...
	private List<IntervalElement<Type>> intervalList;
	...
	public IntervalTree() {
		...
		this.intervalList = new ArrayList<IntervalElement<Type>>();
		this.inSync = true;
		this.size = 0;
	}
	 ...
	 */
	public IntervalTree(List<IntervalElement<Type>> intervalList) {
	...
	public List<Type> get(long time) {
		List<IntervalElement<Type>> intervals = getIntervals(time);
		List<Type> result = new ArrayList<Type>();
		for(IntervalElement<Type> interval : intervals)
	 ...
	 */
	public List<IntervalElement<Type>> getIntervals(long time) {
		build();
	 ...
	 */
	public List<Type> get(Interval searchInterval) {
		List<IntervalElement<Type>> intervals = getIntervals(searchInterval);
		List<Type> result = new ArrayList<Type>();
		for(IntervalElement<Type> node : intervals)
	 ...
	 */
	public List<IntervalElement<Type>> getIntervals(Interval interval) {
		build();
		return head.query(new IntervalElement<Type>(interval, null));
	}
	 ...
	 */
	public void addInterval(IntervalElement<Type> interval) {
	 ...
	 */
	public void addInterval(Interval interval, Type data) {
		intervalList.add(new IntervalElement<Type>(interval, data));
		inSync = false;
	}

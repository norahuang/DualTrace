		...
		if (event == null || !isVisibleMasterDocumentSameAsDocument()) {
			...
			if (commandQueue.passedRedrawCostsThreshold()) {
				...
				try {
					...
					if (!fireRedraw){
						// This happens when fireRedraw is *true* inside executeProjectionCommands()
						// the INVALDIATE_PRESENTATION type of ProjectionCommand will always lead to this,
						// which based on the name makes sense. But this doubles our highlighting requirement...
						// Don't notice a speed or behavior diff when I comment out, so will leave it.

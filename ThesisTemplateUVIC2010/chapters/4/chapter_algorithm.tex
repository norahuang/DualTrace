...
\For{$ev \in etr$}{
   ...
   \If{$ev$ is a function call to $accept$}{
      ...
      \If{$tempstreams[h]$ exist And $tempstreams[hc]$ not exist}{
         ...
         \tcp*[f]{append all the events in the parent stream}\;  
...
\For{$s_0 \in str_0$}{
   $id_0 \leftarrow$ get the channel identifier from $str_0[0].paras$;\;\tcp*[f]{The first event is the open event}\;
   \For{$s_1 \in str_1$}{
      $id_1 \leftarrow$ get the channel identifier from $str_1[0].paras$;\;\tcp*[f]{The first event is the open event}\;
     \If{$id0 equal to id1$}{
         $DataVerified = dataVerify(s_0, s_1, outputdata)$;\; \tcp*[f]{The data verification algorithm}\;

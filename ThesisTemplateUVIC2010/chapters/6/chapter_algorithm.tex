...
\SetKwProg{Pn}{Function}{:}{\KwRet}
  \Pn{\FrebuildSendAndRecv{$trace1,trace2,channels$}}{    
    $trace1Sends \leftarrow List\langle Function\rangle $\; 
    $trace2Sends \leftarrow List\langle Function\rangle $\; 
    $trace1Receives \leftarrow List\langle Function\rangle $\; 
    $trace2Receives \leftarrow List\langle Function\rangle $\;   
        \While{not at end of trace1}{
            \If{socket send function call}{
                 $handle \leftarrow$ handle parameter of the function call\;
                 $sr \leftarrow getSocketSR\left(handle,channels,True\right)$\;                                                   
                 \If{$sr != null$ AND $sr.socket.create.lineNum < currentline$ AND $sr.socket.bind.lineNum < currentline$ AND $sr.socket.connect.lineNum < currentline$ AND $sr.socket.close.lineNum > currentline$}{
                     $send.lineNum \leftarrow currentline$\;
                     $send.bytes \leftarrow$ data that sent by the function call\;   
                     $send.hanle \leftarrow handle$          
                     $trace1Sends.add(send)$
                 }
            } 
            \If{socket receive function call}{
                 $handle \leftarrow$ handle parameter of the function call\;
                 ...
                 \If{$sr != null$ AND $sr.socket.create.lineNum < currentline$ AND $sr.socket.bind.lineNum < currentline$ AND $sr.socket.connect.lineNum < currentline$ AND $sr.socket.close.lineNum > currentline$}{
                     $receive.lineNum \leftarrow currentline$\;
                     $receive.bytes \leftarrow$ data that received when the function return\;              
                     $receive.hanle \leftarrow handle$ 
                     $trace1Receives.add(receive)$
                 }
            }
  }

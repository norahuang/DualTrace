...
\For{$event \in event\_trace$}{
   \If{$event$ is a channel open event}{
      ...
      \If{$endpoint$ is null}{
         $stream = New \enspace EndPoint\left( \right)$;\;
   ...
   \For{$stream1 \in stream\_trace_{1}$}{
     ...
     \If{$channelId0 == channelId1$}{
         ...
         \If{$DataVerified == True$}{
            ...
            $communication.dataMatch = outputdata$ The output from data verification algorithm;\;
...
\subsection{Transmitted Data Verification for Named Pipe and TCP by Data Union: $dataVerify()$}
...
\subsection{Transmitted Data Verification for MSMQ and UDP by Data of Events:$dataVerify()$}
